...
class Admin extends CI_Controller
{
    ...
    public function employee()
    {
        ...
        $this->load->model('Admin_model', 'getuser');
        $data['getuser'] = $this->getuser->getalluser();
    ...
    public function employeeAdd()
    {
        ...
        $this->load->model('Admin_model', 'tyrole');
        $this->load->model('Admin_model', 'sbrole');
        $data['tyrole'] = $this->tyrole->getRole();
        $data['sbrole'] = $this->sbrole->getRole();
        $data['tyrole'] = $this->db->get('user_role')->result_array();
        $data['sbrole'] = $this->db->get('user_sub_role')->result_array();
        $data['user'] = $this->db->get_where('user', ['username' => $this->session->userdata('username')])->row_array();
        //echo 'selamat datang user ', $data['user']['nm_user'];
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data = [
                ...
                'sb_user' => $this->input->post('sb_user'),
                'ac_user' => 1,
                'dt_user' => time()
            ];
    ...
    public function employeeDit($id_user)
    {
        $data['title'] = 'Employee View';
        $data['user'] = $this->db->get_where('user', ['username' => $this->session->userdata('username')])->row_array();
        //echo 'selamat datang user ', $data['user']['nm_user'];

        $data['empviw'] = $this->Admin_model->getuserbyid($id_user);

        $this->load->view('templates/user_header', $data);
        $this->load->view('templates/user_sidebar', $data);
        $this->load->view('templates/user_topbar', $data);
        $this->load->view('admin/employeedit', $data);
        $this->load->view('templates/user_footer', $data);
    }

    public function employeeViw($id_user)
    {
        $data['title'] = 'Employee View';
        ...
        $this->load->view('templates/user_sidebar', $data);
        $this->load->view('templates/user_topbar', $data);
        $this->load->view('admin/employeeviw', $data);
        $this->load->view('templates/user_footer', $data);
    }

    public function employeeDel($id_user)
    {
        $this->Admin_model->deleteuser($id_user);
        $this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">User Removed</div>');
